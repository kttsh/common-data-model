...
- 作成日: 2026-05-31
- 更新日: 2026-06-14（FastAPI のデプロイ先を Azure App Service から ARO〔Azure Red Hat OpenShift〕上のコンテナへ変更。2026-06-11: Dataform デプロイを暫定 = CLI／本筋 = GCP ネイティブの二段構えに改定。リポジトリ名を `common-data-model` に統一、outputs をフラット構成へ追従）

---

## ロードマップ（暫定 → 本筋）

Dataform のデプロイ機構は段階的に進化させる。Step 2〜3 の CLI 方式が**暫定**、Step 4 の GCP ネイティブ方式が**本筋**。

| Step | デプロイ機構 | インフラ管理 | 位置づけ |
|---|---|---|---|
| 1 | 手動（ローカル CLI） | 手動コンソール | 構造集約の検証。移行手順は `../06-data-platform/migration-plan.md` |
| 2 | CLI（GitHub Actions） | 手動（WIF・環境別 CI 用 SA・IAM はここで必須化） | **暫定**。同一 commit の昇格を再現可能にする |
| 3 | CLI（変更なし） | Terraform（`infra-gcp`。dataset・SA・WIF・IAM を import でコード化） | 暫定の継続。ドリフト排除・再現性 |
| 4 | GCP ネイティブ（Terraform 管理の release / workflow config） | Terraform（release / workflow config を追加管理） | **本筋**。正本は `bigquery-dataform-design-rules.md` |
...
| 成果物 | 内容 | デプロイ先 | デプロイ手段 |
|---|---|---|---|
| Dataform | `SQLX`、`workflow_settings.yaml` 等 | BigQuery (GCP) | 暫定: GitHub Actions（CLI）→ 本筋: Terraform 管理の release / workflow config |
| FastAPI | BigQuery のテーブルを参照する Python アプリ | ARO（Azure Red Hat OpenShift）上のコンテナ | GitHub Actions（ACR push → ARO デプロイ） |
...
- **デプロイの軸 = 技術 / クラウド**。Dataform→BigQuery(GCP)、FastAPI→ARO（Azure）と、ランタイムも native 連携も別。
...
- デプロイは**コンテナ方式に一本化**（Dockerfile → ACR へ build-once push → 同一 digest を ARO へ）。FastAPI はコンテナ CMD で `gunicorn -k uvicorn.workers.UvicornWorker` 起動。
- 入口は OpenShift の Route / 内部 LB（private ingress）で、その手前に既存共有 APIM を置く。デプロイ戦略は rolling / blue-green（Route 切替）/ canary。
- GitHub Actions → Azure 認証は service principal シークレットより **OIDC（federated credentials）** が推奨。クラスタへの適用手段（`oc` / Helm / Kustomize 等）は別途確定。
...
- 短所: Invoice 追加が複数リポジトリ・複数 PR に分散。**スキーマ drift 対策が必須**。
...
| 軸 | 選択肢 | 本設計の決定 |
|---|---|---|
| リポジトリ軸 | monorepo / polyrepo | **デプロイ先で分割**（`common-data-model` と `api` は別リポ＝案 B） |
| デプロイ軸 | 単一デプロイ / 複数デプロイ | **api は単一デプロイ**（ARO 上の単一 Deployment / サービス） |
...
- **monorepo ≠ monolith、そして今は分割もしない**: FastAPI Best Practices（Netflix Dispatch 由来）は「多ドメインのモノリスは**ファイル種別別でなくドメイン別**ディレクトリに」と明言。各ドメインが router/schemas/service/repository を自己完結で持つ（後述の構成はこれに準拠）。
...
### repo: `common-data-model`（Dataform → BigQuery）

```
common-data-model/
├── definitions/
│   ├── sources/                    # declaration。源泉システム別サブフォルダ（sap/ coupa/ …）
│   ├── intermediate/               # Silver: dim_*/fct_* の .sqlx（API非公開）
│   └── outputs/                    # ★API公開面（Gold）= 業務名の .sqlx をフラットに置く
│       ├── order.sqlx
│       ├── requisition.sqlx
│       ├── invoice.sqlx
│       └── man_hour.sqlx
...
├── workflow_settings.yaml          # 環境非依存: defaultProject(dev基準)/Dataset/Location, dataformCoreVersion, vars
├── package.json
│   # ★環境差（dev/stg/prod）の注入: 暫定 = CLI コンパイラオプション（--default-database 等）、
│   #   本筋 = Terraform 管理のリリース構成（code_compilation_config）。
│   #   environments/*.json は使うとしてもローカル用で「非コミット」。
└── .github/workflows/
    ├── ci.yml                      # PR: dataform compile + dry-run + assertions
    ├── deploy-dev.yml              # main push: dev PJ へ CLI run（Step 2 で追加・暫定）
    └── promote.yml                 # 承認ゲートで stg→prod（同一commit・暫定はCLI）
```

責務: BigQuery のデータモデル定義・変換・品質保証（assertions）と、その BigQuery へのデプロイ。`outputs/<entity>.sqlx` が API の公開面に対応する。上の図はリポジトリ分割・CI の観点で示したもので、**層構造・テーブル命名・環境分離の正本は [`bigquery-dataform-design-rules.md`](bigquery-dataform-design-rules.md)**（カラム・assertions 等の詳細命名は [`../06-data-platform/dataform-naming-convention.md`](../06-data-platform/dataform-naming-convention.md)）。

### repo: `api`（FastAPI + 埋め込み PEP/PDP → ARO コンテナ）
...
│   │   ├── engine.py               #   埋め込みPDP（PyCasbin Enforcer。確定）
│   │   ├── context.py              #   判断属性の組み立て
│   │   └── policies/               #   ★ポリシー本体＝エンティティ単位で増やす
│   │       ├── model.conf          #     PERM モデル（共通・PyCasbin）
│   │       ├── common.csv          #     テナント分離など共通ポリシー
│   │       ├── order.csv / invoice.csv / …
...
└── .github/workflows/
    ...
    ├── deploy-dev.yml              # main push: ACR push → dev ARO へ同一digest
...
責務: スキーマ/モデル検討の Markdown と、機械可読なテーブル契約（列名・型・PII タグ等）。Dataform と API の間の seam（接合面）。採用するなら outputs の列定義と api の schemas の整合を CI で突き合わせる。

### repo: `infra-gcp`（Terraform → GCP インフラ）

責務: GCP のフォルダ・プロジェクト・データセット・IAM・Dataform の器（repository / release config / workflow config）。ロードマップ Step 3 以降で導入し、内部構成の正本は [`bigquery-dataform-design-rules.md`](bigquery-dataform-design-rules.md)。
...
1 つのビジネスエンティティは、公開 URL・API コード・Dataform 出力の 3 面で**同じドメイン境界を 1:1 で表現**する。新エンティティ追加はこの 3 面（＋認可ポリシー）に同名で足すだけ（API は 1 ディレクトリ、Dataform とポリシーは 1 ファイル）。

| エンティティ | URL（公開契約） | API ディレクトリ | Dataform 出力 | 認可ポリシー |
|---|---|---|---|---|
| Invoice | `/v1/invoices` | `src/invoice/` | `definitions/outputs/invoice.sqlx` | `src/authz/policies/invoice.csv` |
| Purchase Order | `/v1/purchase-orders` | `src/purchase_order/` | `definitions/outputs/purchase_order.sqlx` | `…/purchase_order.csv` |
| Man Hour | `/v1/man-hours` | `src/man_hour/` | `definitions/outputs/man_hour.sqlx` | `…/man_hour.csv` |
| Cost | `/v1/costs` | `src/cost/` | `definitions/outputs/cost.sqlx` | `…/cost.csv` |
...
### 増やす（エンティティ追加ごとに加算）
- Dataform: `definitions/outputs/<entity>.sqlx`（必要に応じて `sources/` の宣言・`intermediate/` の `dim_*`/`fct_*`）
- FastAPI: `src/<entity>/` パッケージ 1 式（router/schemas/service/repository/…）
- 認可: `src/authz/policies/<entity>.csv`
...
- 認可: PDP ランタイム（PyCasbin Enforcer・埋め込み）、`model.conf`（PERM モデル）、`common.csv`（テナント分離など共通ポリシー）

新エンティティ追加時に作るのは `outputs/<entity>.sqlx`・`src/<entity>/`・`policies/<entity>.csv` の 3 点セットのみ。これによりレビュー範囲とデプロイ範囲が追加分に局在する。
...
| 層 | dev | stg | prod |
|---|---|---|---|
| GCP | cdm-dev（源泉 PJ も同様に 3 面） | cdm-stg | cdm-prod |
| Azure | ARO(dev) | ARO(stg) | ARO(prod) |

- **ランドスケープごとに独立した ARO 環境**（専用クラスタ、または共有クラスタ内の独立 namespace。dev は小さいノード構成可。共有/専用の方針は未決）。3 ランドスケープと、prod 内の「Route 切替による無停止 blue/green」は別概念。後者は必要なら prod 内にだけ足す。
- 各 ARO 環境が参照する GCP プロジェクトは **GitHub Environments の変数/シークレットで注入**（実体は ConfigMap / Secret 等）。dev ARO → cdm-dev を固定対応。
- 認証は鍵レスで統一: GitHub Actions→GCP は Workload Identity Federation(OIDC)、GitHub Actions→Azure は federated credentials(OIDC)。ARO 上の Pod 実行時の BigQuery 参照認証（Azure→GCP）は実装で詰める論点（WIF か、最小権限 SA の安全な保持か）。
...
2. **Fix forward at the source. 環境を直接いじらない。** stg の BigQuery テーブルを手で直す／stg の Pod・クラスタに入って patch する、は禁止。直すのは常にリポジトリのコード。
...
**B. 環境設定/インフラの不具合（env var 誤り / IAM 不足 / dataset 名 / ARO マニフェスト・設定）**
- 治す場所: config-as-code（GitHub Environment 変数、Terraform〔GCP インフラ・リリース構成〕、bicep〔Azure / ARO クラスタ〕、k8s マニフェスト〔ARO デプロイ: Deployment/Route/ConfigMap 等〕）。手動変更しない。
...
hotfix の切り出し元（main から切るか、prod タグから切るか）の場合分けは [§10.5 hotfix の扱い（精密版）](#105-hotfix-の扱い精密版) に集約。いずれも必ずパイプライン経由で、環境（BigQuery テーブルや ARO 上の Pod / クラスタ）の直接編集はしない。
...
- **Dataform のデプロイ方式（二段構え）**: **暫定 = CLI 方式**。GitHub Actions が `@dataform/cli` で対象 PJ の BigQuery に run し、環境差は `--default-database` 等の CLI オプションで注入する（移行手順は `../06-data-platform/migration-plan.md`）。**本筋 = GCP ネイティブ方式**。Terraform 管理の release / workflow config（環境差は `code_compilation_config`）へ、`infra-gcp` 整備後に移行する（正本は `bigquery-dataform-design-rules.md`、基礎検討は `../06-data-platform/dataform-operating-model.md`）。いずれも「同一 commit→dev→stg→prod」を表現する。スケジューリング（定期リフレッシュ）はデプロイとは別決定として後で詰める。
...
- [ ] 暫定（CLI 方式）の環境オーバーライド具体フラグを、導入する `@dataform/cli` のバージョンに合わせて確定。
- [ ] 本筋（GCP ネイティブ）移行時の昇格手順を確定（release config の `git_commitish` をタグ/SHA で固定し Terraform に反映する流れ）。
- [ ] ARO 上の Pod 実行時の Azure→GCP 認証方式（WIF か最小権限 SA か）の決定。
- [ ] ARO クラスタのトポロジ（ランドスケープごとに専用クラスタか、共有クラスタ + namespace 分離か）の決定。
- [ ] ARO へのデプロイ適用手段（`oc` / Helm / Kustomize）と、Route / 内部 LB と既存 APIM の接続方式の決定。
- [ ] 補助バッチ（マテビューウォーマー / Webhook 等）のランタイム（ARO の CronJob/Job か Azure Functions か）の決定。
- [x] 埋め込み PDP の評価エンジン選定 → **PyCasbin（埋め込み）で確定**（`../04-research/abac-authz-library-comparison.md`、`../03-authorization/pycasbin/`）。外部 PDP（Cerbos/OPA）は将来オプション。残りは `context.py` の入力属性設計。
...
昇格モデル（trunk-based + GitHub Environments 承認ゲート、build-once / fix-forward）と整合させるため、**全リポジトリをトランクベースで統一**する。`main` を唯一のトランク（保護ブランチ）とし、そこから短命の feature ブランチを切って PR で戻す。
...
非採用判断: **branch-per-environment（`dev`/`stg`/`prod` ブランチ）は採らない**。環境ごとのコード分岐を誘発し、build-once / fix-forward を壊すため。暫定の CLI 方式は `main` の同一 commit を各環境へ run し、本筋の GCP ネイティブでも release config の参照先を `main` ＋昇格時のタグ/SHA 固定とすることで、環境ブランチを作らない。

### 10.2 repo: `common-data-model`（Dataform）
...
- main マージ → image を ACR に immutable tag（git SHA）で push し、その digest を dev ARO へ。昇格は同一 digest を stg → prod
...
- いずれも必ずパイプライン経由。環境（BigQuery テーブルや ARO 上の Pod / クラスタ）の直接編集はしない

### 10.6 リポジトリ間の協調

案 B で各リポジトリは独立したバージョン線を持つ。「Invoice 追加」のように複数リポジトリへまたがる変更は、共有ブランチではなく **contract のバージョン + オープン事項プロセス**で協調する（contract bump → dataform 追従 PR → api 追従 PR）。
...
- Azure Red Hat OpenShift: ドキュメント（クラスタ・ネットワーク・運用）— https://learn.microsoft.com/en-us/azure/openshift/
- OpenShift Deployments（rolling / blue-green / canary）— https://docs.openshift.com/container-platform/latest/applications/deployments/deployment-strategies.html
