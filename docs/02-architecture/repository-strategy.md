...
├── package.json
│   # ★環境差（pj-dev/stg/prod）はデプロイ時の CLI コンパイラオプション（--default-database 等）で注入する。
│   #   environments/*.json は使うとしてもローカル用で「非コミット」（CLI 路線。migration-plan.md と整合）。
└── .github/workflows/
    ├── ci.yml                      # PR: dataform compile + dry-run + assertions
    ├── deploy-dev.yml              # main push: dev PJ へ CLI run（Step 2 で追加）
...
|---|---|---|---|---|
| Invoice | `/v1/invoices` | `src/invoice/` | `definitions/outputs/invoice/` | `src/authz/policies/invoice.csv` |
| Purchase Order | `/v1/purchase-orders` | `src/purchase_order/` | `definitions/outputs/purchase_order/` | `…/purchase_order.csv` |
| Man Hour | `/v1/man-hours` | `src/man_hour/` | `definitions/outputs/man_hour/` | `…/man_hour.csv` |
| Cost | `/v1/costs` | `src/cost/` | `definitions/outputs/cost/` | `…/cost.csv` |
...
新エンティティ追加時に作るのは `outputs/<entity>/`・`src/<entity>/`・`policies/<entity>.csv` の 3 点セットのみ。これによりレビュー範囲とデプロイ範囲が追加 1 ディレクトリに局在する。
...
- **Dataform のデプロイ方式**: **CLI 方式で確定**。GitHub Actions が `@dataform/cli` で対象 PJ の BigQuery に run し、「同一 commit→dev→stg→prod」を最も素直に表現する（環境差は `--default-database` 等の CLI オプションで注入）。GCP ネイティブ（release/workflow configuration＋strict act-as カスタム SA）方式は **不採用**（検討記録は `../06-data-platform/dataform-operating-model.md`、移行手順は `../06-data-platform/migration-plan.md`）。スケジューリング（定期リフレッシュ）はデプロイとは別決定として後で詰める。
