...
データ基盤側のリポジトリは Dataform 用（`common-data-model`）と Terraform 用（`infra-gcp`）の2つに分離する（理由は §1 参照）。API を含むプラットフォーム全体のリポジトリ戦略は `repository-strategy.md` を正本とする。
...
dataformCoreVersion: 3.0.52             # 例。レンジ指定せず固定値でピン留めする
vars:
...
resource "google_dataform_repository_release_config" "prod" {
  ...
  region        = var.location
  name          = "prod"
  git_commitish = var.prod_release_ref # 昇格時にタグ/SHAで固定（mainのHEAD追従にしない。§6-4）
...
環境切替は二段構え。**本筋**は以下のリリース構成方式（Terraform 管理）、**暫定**（リリース構成の整備前）は CLI の `--default-database` 等によるコンパイラオプション注入で、同じ「コード1セット・環境差は注入」を実現する（暫定運用の手順は `repository-strategy.md`・`../06-data-platform/migration-plan.md`）。
...
| # | 論点 | 推奨 | メモ |
|---|---|---|---|
| 1 | 洗浄層の置き場所 | 共通モデル側に `stg_*` を置く（下記詳細） |クレンジングはsilverで行う|
| 2 | リージョン戦略 | 全プロジェクト・全データセットで単一リージョンに統一 |統一|
| 3 | クロスプロジェクトIAM | Authorized Dataset で源泉Bronzeを共通モデルSAに限定公開 |raw dataをbronzeに取り込んじゃう|
| 4 | 環境間プロモーション機構 | Terraform管理のリリース構成 + GitHub Environments承認ゲート |世の中の推奨案で織り込む|
| 5 | データ品質自動化 | Dataformアサーションを必須化しCIゲートに組み込む |同様に織り込む　CIを段階的に手厚くしていく。世の中のプラクティスを調べる|
| 6 | パーティション/コスト制御 | 日付パーティション + クラスタリング + bytes上限クォータ |コスト制御は別だてで検討。|
| 7 | 履歴管理（SCD） | まずSCD Type1 + 日次スナップショットで開始 |一旦なし|
| 8 | データセット名前空間 | 当面2データセット維持、ドメイン増加時に分割基準を再判定 |OK|
| 9 | レイヤー別オーナーシップ | Bronze=連携チーム、Silver/Gold=共通モデルチームと明文化 |これは後回し|
 
### 6-1. 洗浄層の置き場所（最優先）
 
**現状の矛盾:** 連携元フォルダは「Bronzeのみ」だが、共通モデルフォルダは「連携フォルダのSilverから取得」と記述されており、Bronze→Silverの洗浄（型変換・NULL処理・重複排除）の責務が未定義。
 
**推奨:** 源泉フォルダはBronze専用を維持し、洗浄は共通モデル側に置く。具体的には `definitions/intermediate/` に `stg_<source>_<table>.sqlx` を配置し、`intermediate` データセット内に実体化する（専用 `staging` データセットを切る場合は `definitions/staging/` を併設し、フォルダ名=データセット名の構成ルールを維持）。フローは `bronze（宣言）→ stg_（洗浄）→ dim_/fct_ → outputs（業務名）`。
...
dev → 検証 → 本番の昇格は「同一コミットの再コンパイル」で実現する。検証・本番それぞれのリリース構成をTerraformで定義し、`git_commitish` は昇格対象のタグ/SHAで固定する（mainのHEAD追従にしない）。本番適用はGitHub Environmentsの承認ゲートを通す。暫定（CLI）期はGitHub Actionsが同一コミットを各環境へrunし、承認ゲートは同じくGitHub Environmentsを用いる。データセットの手動コピーによる昇格は禁止。
