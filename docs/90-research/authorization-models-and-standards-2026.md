...
PDP の置き方は、`docs/02-architecture/platform-architecture-decision.md` の制約と擦り合わせる必要がある。

> 注記（2026-06-14）: 本節は調査時点（バックエンドは App Service / Functions のみ・Container Apps / AKS 不可）の前提で書かれている。その後 **C1 は改定され、ランタイムは ARO〔コンテナ〕に変更**（サイドカー可）。以降の「App Service との相性」表は当時の制約下の評価として読むこと。最新は owner（`platform-architecture-decision.md`）が正。
