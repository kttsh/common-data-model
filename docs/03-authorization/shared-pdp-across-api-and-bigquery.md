...
> 前提: 認証=Entra ID、認可属性=Open-GIM、データ=BigQuery、API=FastAPI/App Service、API→BigQuery は単一 SA、認可は API 層 ABAC を正本
> **重要（整合）**: 認可エンジンは **PyCasbin（埋め込み）で確定済み**（`../04-research/abac-authz-library-comparison.md`、`pycasbin/`）。本メモが第一候補とする **外部 PDP（Cerbos/OPA）は将来オプション**であり、現時点の採用ではない。本メモは「PDP を複数経路で共用する」要件が確定したときに備えた検討材料として読むこと。
> Related: `row-level-filtering-layering.md`, `authorization-boundaries-and-interface.md`, `../04-research/authorization-models-and-standards-2026.md`
> 情報時点: 2026年6月
...
- 現行の採用は **PyCasbin（埋め込み）**。**経路間共用を主目的に格上げする場合の将来オプション**として言えば、**外部 PDP（Cerbos / OPA を別ホスト）** が最も素直（ポリシー更新が 1 か所で済む）。App Service はサイドカー非対応のため、その場合は **PDP を別 App Service / Function にホストし、1 ホップを短 TTL キャッシュで吸収**するのが現実解。埋め込み PyCasbin のままでも、後述②③の共有ライブラリ化で経路間の挙動ズレは抑えられる。
...
- WHERE は文字列連結で組まず、ユーザー由来の値は BigQuery のクエリパラメータ（`@param`、配列は `UNNEST(@allowed_depts)`）で渡す（`row-level-filtering-layering.md` §5 と整合）。
...
| 方式 | 共用のしやすさ | レイテンシ | App Service との相性 |
|---|---|---|---|
| **埋め込み（PyCasbin 等／プロセス内）** | △ 別プロセス/別デプロイごとにライブラリと最新ポリシーを配る必要。配布同期が課題 | ◎ プロセス内 | ◎ そのまま動く |
| **外部 PDP（Cerbos / OPA を別ホスト）** | ◎ 両経路が同じエンドポイントを叩く。ポリシー更新は 1 か所 | ○ ネットワークホップ 1 回（短 TTL で吸収） | △ サイドカー非対応 → 別 App Service / Function にホスト |

- 共用を主目的にするなら **外部 PDP が有利**。Cerbos はステートレスでサブミリ秒評価、エアギャップ/閉域環境にも対応し、サーバーレス関数やサイドカー形態で自己ホストできるため、Azure Functions / App Service で別ホストとして立てられる。
- App Service はサイドカー非対応なので、**PDP を別ホストに置き、1 ホップを短 TTL キャッシュで吸収**して 100〜500ms 目標を守る（`authorization-models-and-standards-2026.md` §5 のデプロイ注意と一致）。
...
「実際に適用した WHERE と判定理由」を残すのは既存方針の肝。`audit.py` を両経路で共有し、JSON 経路でもファイル経路でも同じスキーマでログする（`row-level-filtering-layering.md` §7、`authorization-boundaries-and-interface.md` §7 と整合）。
...
1. **現行は PyCasbin（埋め込み）で確定**。経路間共用が将来ハード要件に格上げされた場合に限り、**外部ホスト型（Cerbos / OPA）を第一候補**として再検討し、ポリシーを唯一の正本にする。App Service 制約は別ホスト＋短 TTL キャッシュで吸収。
...
| # | 項目 | 内容 |
|---|---|---|
| 1 | PDP エンジン選定 | D2 は **PyCasbin（埋め込み）でクローズ済み**。本項は将来 PDP を外部化する場合に限り、Cerbos と OPA のどちらが BigQuery 翻訳層を書きやすいかを具体比較する |
| 2 | 翻訳層実装 | PlanResources / Compile 出力例 → BigQuery パラメータ化 SQL のマッピング設計・実装イメージ |
| 3 | 外部 PDP のホスティング | Azure 閉域での PDP ホスト形態（別 App Service / Function）と疎通・キャッシュ TTL |
| 4 | 語彙設計 | action/resource/context の共通語彙定義（read と出力先 context の分離） |
| 5 | 直接接続要件の確定 | 利用者アプリの「DB 接続で BQ」の正体（任意 SQL 可否・接続 ID）を確認し、①/②の分岐を決める |
| 6 | PoC | 「部門費モデル」で subject→属性→PDP→AST→生成 WHERE→BigQuery 実行→列マスク を、FastAPI と抽出処理の両経路で共有ライブラリ経由で最小実装 |
