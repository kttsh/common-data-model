...
- 作成日: 2026-06-05
- 位置づけ: `migration-plan.md`（移行の順番・原則・名称マッピングの**正本**）と `dataform-naming-convention.md`（命名・層構造の**正本**）の派生。本書は実装作業の手順書であり、両正本と矛盾する場合は正本が優先する。
...
`migration-plan.md` のロードマップ Step 1（手動・ローカル CLI）で構造集約を完結させる範囲。CI 自動デプロイ・WIF・Terraform（Step 2／3）は対象外。
...
| 3 | `feature/cost-outputs` | 2＋3 | cost の 3 環境コピーを 1 本化し `outputs/cost/` へ（名前は維持） |
...
## 4. `feature/cost-outputs`【Phase 2＋3】

### 目的
cost ドメインの **3 環境コピー（`*` / `*-stg` / `*-prd`）を 1 本に畳み**、`outputs/cost/` へ 3 層配置する。**テーブル名は現状維持**（大文字のまま）。`dataform-rewrite.sh`（`sed` 置換）と REST `writeFile` への依存をこのドメインから外す。
...
- [ ] master → `outputs/cost/cost_master.sqlx`、detail → `outputs/cost/cost_detail.sqlx` に集約。
...
|---|---|---|---|
| `Dim_COST_MASTER{,-stg,-prd}` | `outputs/cost/cost_master.sqlx` | `Dim_COST_MASTER` | `cost_master` |
| `Fact_COST_DETAIL{,-stg,-prd}` | `outputs/cost/cost_detail.sqlx` | `Fact_COST_DETAIL` | `cost_detail` |

### ファイルの中身（雛形）

`definitions/outputs/cost/cost_master.sqlx`（**名前維持**パターン。消費者ありで Phase 4 まで旧名を保つ）:
```sqlx
config {
  type: "table",
  schema: "outputs",
  name: "Dim_COST_MASTER",   -- Phase 2+3 は名前維持。消費者なしなら "cost_master" にしてこの行を削除可
...
`definitions/outputs/cost/cost_detail.sqlx`:
```sqlx
config {
  type: "table",
  schema: "outputs",
  name: "Fact_COST_DETAIL",  -- 同上（消費者なしなら "cost_detail"）
...
（任意）スタンドアロン assertion を切る場合 `definitions/outputs/cost/assert_cost_detail_unique.sqlx`:
```sqlx
config { type: "assertion" }
SELECT <pk_col>, COUNT(*) c
FROM ${ref("Fact_COST_DETAIL")}   -- 維持中の名前 / リネーム後は cost_detail
...
## 5. `feature/order-outputs`【Phase 2＋3】
...
- [ ] master → `outputs/order/order_master.sqlx`、detail → `outputs/order/order_detail.sqlx` に集約（名前維持）。
...
|---|---|---|---|
| `Dim_ORDER_MASTER{,-stg,-prd}` | `outputs/order/order_master.sqlx` | `Dim_ORDER_MASTER` | `order_master` |
| `Fact_ORDER_DETAIL{,-stg,-prd}` | `outputs/order/order_detail.sqlx` | `Fact_ORDER_DETAIL` | `order_detail` |

### ファイルの中身（雛形）

`definitions/outputs/order/order_master.sqlx`:
```sqlx
config {
  type: "table",
  schema: "outputs",
  name: "Dim_ORDER_MASTER",   -- 消費者なしなら "order_master"
  description: "注文マスタ",
  assertions: {
    nonNull: ["order_id"],
    uniqueKey: ["order_id"],
...
`definitions/outputs/order/order_detail.sqlx`:
```sqlx
config {
  type: "table",
  schema: "outputs",
  name: "Fact_ORDER_DETAIL",  -- 消費者なしなら "order_detail"
...
- [ ] 新名（例 `cost_master`）の `type: "table"` を作成（中身は既存のまま `name` を新名に）。
...
新名テーブル `definitions/outputs/cost/cost_master.sqlx`（`name` を新名へ。`config.name` 省略でファイル名＝テーブル名でも可）:
```sqlx
config {
  type: "table",
  schema: "outputs",
...
後方互換ビュー `definitions/outputs/cost/legacy_dim_cost_master.sqlx`（**一時的**。旧名で新名を指す）:
```sqlx
config {
  type: "view",
  schema: "outputs",
  name: "Dim_COST_MASTER",     -- 旧名を view として一時的に温存
  description: "後方互換: 旧名 Dim_COST_MASTER → 新 cost_master。下流移行後に撤去",
}
SELECT * FROM ${ref("cost_master")}
```

> order 側も同型（`order_master` / `order_detail` を新名、`Dim_ORDER_MASTER` / `Fact_ORDER_DETAIL` を後方互換 view）。
...
| 対象 | 理由 |
|---|---|
| `empty/` | 空 Dataform repo の雛形（CLI 路線で不要） |
...
│   ├── intermediate/stg_*.sqlx       # 中間変換（API 非公開）
│   └── outputs/<domain>/*.sqlx       # 出力＝API 公開面（cost/ order/ …）
...
- [ ] 命名規約（小文字 snake_case・`dim_/fct_` なし）を満たす（#5 完了が前提）。
...
- CLI 路線を選んだ検討記録: [`dataform-operating-model.md`](dataform-operating-model.md)
