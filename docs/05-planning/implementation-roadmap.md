...
- Adopted platform baseline: ARO (Azure Red Hat OpenShift) containers + existing APIM + APIM built-in cache + in-process LRU + BigQuery views (switched from App Service, 2026-06).
- Authorization baseline: the API (FastAPI on ARO) owns per-user ABAC decisions; BigQuery executes pushed-down parameterized filters; APIM remains the coarse entry gate.
...
1. Confirm Proxy specifications and run an ARO-to-BigQuery connectivity PoC (incl. egress lockdown / UDR through the corporate Proxy).
2. Confirm ARO cluster sizing (node pool SKU/count, Pod resources, HPA), Route/internal-LB-to-APIM integration, and private-cluster policy.
...
2. APIM -> ARO (FastAPI container) -> BigQuery via Proxy latency measurement.
...
2. **Phase 1(PoC・4〜6週間)**
   - 選定言語で最小エンドポイント(`/healthz` + BigQuery 簡易クエリ + Entra ID JWT 検証)を実装、ARO（コンテナ）にデプロイ。
   - GitHub Actions の CI/CD 雛形(.github/workflows/ci.yml + deploy-dev.yml)を1本動かす(build → ACR push → ARO へ rolling / blue-green デプロイ)。Harness は採用見送り(`../04-research/ci-cd-delivery-research-2026.md`、将来比較用)。
...
3. **Phase 2(本格構築・8〜12週間)**
   ...
   - APIM Policy で JWT 事前検証(発行者・audience)+ ARO 上の FastAPI で再検証(claims ベース ABAC 用)。
4. **しきい値(判断を変える基準)**
...
6. **Bun / Elysia / Encore.ts**: 旧 App Service 制約下では実用面で除外していたが、**ARO（コンテナ）採用でこの実行基盤制約は解消**。ただし Python / FastAPI の採用判断（AI 支援開発相性・エコシステム成熟度）は不変のため引き続き非採用とし、再評価は言語選定の再検討トリガー発生時に限る。
