...
- データソースは BigQuery。
- API 実行層は Azure App Service を中心にし、既存共有の Azure API Management を入口にする。
- キャッシュは APIM 組込みキャッシュと App Service プロセス内 LRU を起点にする。
- 認証は Entra ID。認可属性は OpenGIM / オンプレ SQL Server 側の属性ストアを参照する。
...
- App Service プラン、Linux/Windows、デプロイスロットの利用制約。
- APIM の Product / Subscription / 命名規約。
- OpenGIM の属性項目、突合キー、到達経路。
