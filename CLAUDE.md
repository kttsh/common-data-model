...
| テスト戦略（基本方針・各層・CI ゲート） | `docs/07-testing/test-concept.md` |
...
フォルダの役割: `01-requirements`（要件）/ `02-architecture`（採用決定）/ `03-authorization`（認可設計、`pycasbin/` に実装メモ）/ `04-research`（調査の生記録、決定しない）/ `05-planning`（ロードマップ）/ `06-data-platform`（Dataform 関連）/ `07-testing`（テスト戦略、横断）。文書一覧は `docs/README.md`。
